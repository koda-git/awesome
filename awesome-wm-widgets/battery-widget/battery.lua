...
local function worker(user_args)
    ...
    function(widget, stdout)
        ...
        for s in stdout:gmatch("[^\r\n]+") do
            local status, charge_str, _ = string.match(s, '.+: (%a+%s?%a*), (%d?%d?%d)%%,?(.*)')
            if status ~= nil then
        ...
        if (charge >= 0 and charge < 15) then
            ...
            if enable_battery_warning and status ~= 'Charging' and status ~= 'Not charging' and os.difftime(os.time(), last_battery_check) > 300 then
